...
import sys, PIL.Image, urllib, httplib, urlparse, StringIO, math, thread, time, os
...
class TileRequest:
    ...
    def to_cache_filename(self, netloc, path):
        netloc = netloc.strip('/')
        path   = path.strip('/')
        dirname = os.path.dirname(path)
        path = os.path.basename(path)
        md5 = hashlib.md5(path).hexdigest()
        return os.path.join(dataCacheDir, netloc, dirname, md5[:2], md5[2:4], path)
    ...
    def load(self, lock, verbose, attempt=1):
        ...
        try:
            ...
            for (scheme, netloc, path, params, query, fragment) in map(urlparse.urlparse, urls):
                ...
                # First, search file in cache
                if dataCacheDir:
                    cfilename = self.to_cache_filename(netloc, path)
                    if os.path.exists(cfilename):
                        if os.path.getsize(cfilename) == 0:
                            continue
                        fd = open(cfilename)
                    else:
                        basedir = os.path.dirname(cfilename)
                ...
                if fd is None:
                    ...
                    if str(response.status).startswith('2'):
                        ...
                        if cfilename:
                            open(cfilename, 'wb+').close()

                if fd:
                    imgs.append(PIL.Image.open(fd).convert('RGBA'))
                    fd.close()
...
class Map:
    ...
    def draw(self, verbose=False, nocenter=False):
        """ Draw map out to a PIL.Image and return it.
        """
        coord = self.coordinate.copy()
        if nocenter:
            corner = Core.Point(int(self.offset.x + self.dimensions.x), int(self.offset.y + self.dimensions.y))
        else:
